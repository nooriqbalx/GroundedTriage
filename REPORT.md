...
I conduct a stratified human evaluation of model justification grounding, applying a strict rule under which an unsupported family-specific attribution disqualifies an otherwise evidence-accurate justification from being counted as grounded, and report the resulting grounding rates by model and evidence condition, together with a descriptive failure-mode taxonomy and an independent OSINT validation of the resulting misattribution claims (Section 5).
...
### 3.7 Failure-Mode Taxonomy and OSINT Validation Protocol

To characterize the dominant failure mode identified in Section 3.6 more precisely, I performed a second pass over the same 45 human-reviewed responses, categorizing each PARTIAL response into one of three descriptive failure modes, developed inductively from the justification texts rather than specified in advance (full protocol in Section 5.4). Before conducting this categorization, I performed a power analysis to determine whether the existing sample could support a formally tested comparison of category distribution across evidence conditions; the results of this analysis, and their consequence for how the taxonomy is reported, are described in Section 5.4.

To validate a subset of the resulting misattribution claims independently of my own judgment, I checked each PARTIAL response in the signature-misattribution and compound-narrative categories for a specifically citable artifact, an import hash or network domain, and where one was present, queried it against MalwareBazaar's public hash database or manually checked it against public threat-intelligence sources, to determine whether the artifact carries any independent association with the family the model named. Full protocol and results are in Section 5.5.
...
### 5.4 Failure-Mode Taxonomy of Ungrounded Justifications

Section 5.3 establishes that the dominant failure mode is not fabrication from nothing but the construction of an unsupported narrative around real evidence. To characterize this failure mode more precisely, I conducted a second pass over the same 45 human-reviewed responses, assigning each PARTIAL or GROUNDED-with-caveats response to one of three descriptive categories, drawn inductively from the justification texts themselves rather than specified in advance: **signature/hash misattribution**, in which a real static artifact such as an import hash or file format is treated as sufficient basis for a specific family attribution without behavioral confirmation; **generic-technique-as-fingerprint**, in which real but common behaviors or MITRE ATT&CK techniques, such as process injection or registry-based persistence, are described as though uniquely diagnostic of a single named family despite appearing across many families; and **compound narrative construction**, in which several individually generic real observations are combined into a specific, confident attribution that no single piece of evidence, nor their combination, actually establishes.

Prior to conducting this categorization, I performed a power analysis to determine whether the existing 45-response sample could support a formally tested comparison of failure-mode distribution across evidence conditions. This analysis found that, at n = 15 per condition, the sample is underpowered to detect even a large effect (Cohen's w = 0.5) across a taxonomy of three or more categories at the conventional 80% power threshold. I therefore report the taxonomy descriptively, as category counts and illustrative examples, rather than as a statistically tested comparison.

**Table 6: Failure-mode category counts by evidence condition (N = 45, descriptive)**

| Category | Static | Dynamic | Combined |
|---|---|---|---|
| Signature/hash misattribution | 3 | 2 | 3 |
| Generic-technique-as-fingerprint | 0 | 7 | 3 |
| Compound narrative construction | 0 | 3 | 4 |
| Other (hedged or malformed) | 1 | 1 | 4 |
| Fully grounded | 11 | 2 | 1 |

![Figure 6: Failure-mode category by evidence condition](figures/fig6_taxonomy.png)

**Figure 6:** Failure-mode category counts by evidence condition (N = 45). Generic-technique-as-fingerprint and compound narrative construction are both absent under static evidence and increase once dynamic evidence is introduced, while signature/hash misattribution remains roughly stable across all three conditions.

Two descriptive patterns are visible in Table 6 and Figure 6, and are consistent with, though not statistically confirmed by, the mechanism I propose below. First, generic-technique-as-fingerprint failures are essentially absent under static evidence and concentrate heavily once dynamic evidence is introduced, suggesting this failure mode is specifically unlocked by the presence of behavioral evidence rather than a general property of richer prompts. Second, compound narrative construction is entirely absent under static evidence and increases monotonically as more evidence types become available, consistent with this failure mode requiring multiple distinct evidence types to weave together. Under this reading, each additional evidence type does not merely provide more opportunities to be wrong in the same way; it introduces a qualitatively distinct route to an ungrounded conclusion.

A secondary, model-level pattern is also worth noting descriptively. GPT-OSS-120B, the model with the strongest accuracy and abstention behavior in Section 4, accounts for zero of the eight signature-misattribution cases, the crudest failure mode identified here; its failures cluster instead in the technique-fingerprinting and compound-narrative categories. GPT-OSS-20B, the model with 0.0% raw accuracy, accounts for five of eight signature misattributions and five of seven compound narratives. This suggests that better-calibrated models may not only abstain more appropriately but, when they do fail, tend to fail through more sophisticated reasoning errors rather than the crudest available shortcut.

### 5.5 Independent Validation of Misattribution Claims

The grounding review in Section 5.1 and the taxonomy in Section 5.4 both rely on human judgment of whether a justification's claims are supported by the evidence shown to the model. To provide an independent, falsifiable check on this judgment, I selected the subset of misattribution cases in which a specific artifact, an import hash or a network domain, was cited as evidence for a specific named family, and checked whether that artifact carries any independent public association with the claimed family. Fifteen cases from the signature-misattribution and compound-narrative categories were reviewed for a checkable artifact; eleven contained one. Of these eleven, four involved an import hash checkable against MalwareBazaar's public hash database, and seven involved a network domain or code-signing certificate checked manually against public threat-intelligence sources.

**Table 7: Independent validation of checkable misattribution claims (N = 11)**

| Outcome | Count | Proportion (95% Wilson CI) |
|---|---|---|
| Independently supported | 0 | 0.0% [0.0%, 25.9%] |
| Directly contradicted | 4 | 36.4% |
| No independent support found | 7 | 63.6% |

None of the eleven checkable claims received independent support. All four imphash-based claims were directly contradicted: in each case, the cited hash was publicly associated with the sample's true family or with a different, unrelated family, never with the family the model named. The seven domain- or certificate-based claims each relied on infrastructure too generic to establish family-specific attribution on its own, including a shared public communication platform, a public cryptocurrency mining pool, and an open-source tunneling tool, each of which is used across many unrelated malware families and legitimate software alike.

Two of the seven manually reviewed cases warrant separate mention. In both, the model attributed the sample to a named family, "Criptor" and "Lumin" respectively, that does not appear as a documented malware family in standard public threat-intelligence sources, including MITRE ATT&CK and major vendor reporting. In both cases, the named family closely echoes a distinctive substring of the cited command-and-control domain (`criptor.io` and `lumm.io`), suggesting the model may have derived a plausible-sounding family name directly from the domain string itself rather than recalling or reasoning about any actual documented family. If this interpretation is correct, it represents a failure mode qualitatively distinct from, and arguably more concerning than, misattribution to a real but incorrect family: rather than confusing one known entity for another, the model appears in these cases to construct an entity that does not exist and present it with the same confident, evidence-citing register used elsewhere for accurate claims.
...
The extended analysis in Sections 5.4 and 5.5 sharpens this picture in two ways. First, the failure-mode taxonomy indicates that the risk introduced by richer evidence is not uniform in kind: static evidence, when misread, produces the comparatively crude error of treating a coincidental hash match as definitive, while dynamic and combined evidence introduce failure modes, technique-as-fingerprint reasoning and compound narrative construction, that are individually more sophisticated and, correspondingly, likely harder for a time-pressured reviewer to catch. Second, the independent OSINT validation in Section 5.5 confirms, using a check entirely external to the author's own judgment, that specific misattribution claims in this dataset carry no independent support, and in at least two cases appear to name a family that does not exist. This last finding extends the practical concern raised in Section 5.3: a reviewer checking whether cited evidence is real would find that it is, in most of these cases, genuinely present in the input; only a further check against external, independent sources would reveal that the family attribution built on top of that evidence is unsupported or, in the "Criptor" and "Lumin" cases, apparently invented.
...
**Taxonomy statistical power.** The failure-mode taxonomy in Section 5.4 is reported descriptively rather than as a statistically tested comparison, following a power analysis indicating that n = 15 per condition cannot support a formally tested category comparison at conventional power thresholds. The condition-level and model-level patterns discussed in Section 5.4 should accordingly be read as suggestive rather than confirmed. Similarly, the OSINT validation in Section 5.5 covers only the eleven of forty-five reviewed responses containing a specifically checkable artifact; the generic-technique-fingerprint and hedged/other categories, comprising the remaining cases, could not be independently checked by this method, since their claims do not rest on a single verifiable artifact.

---

## 8. Conclusion

This study set out to answer a simple question with a real practical stake: does giving a large language model more evidence make it a more reliable malware triage assistant? Across 504 evaluated responses spanning three models, three evidence conditions, and 56 real-world malware samples, the answer found here is no, at least not straightforwardly. Classification accuracy remains flat regardless of how much evidence a model is given. What changes, sharply and consistently across all three models tested, is the model's willingness to commit to a confident answer rather than acknowledge uncertainty, and a human-verified evaluation of 45 model justifications shows that this growing confidence is largely unearned: grounding of model justifications in the actual evidence shown collapses from 73.3% to 6.7% across the same progression from sparse to rich evidence. A descriptive taxonomy of these justification failures further shows that the character of ungrounded reasoning shifts as evidence richens, from crude static-artifact misattribution toward more sophisticated technique-fingerprinting and compound narrative construction, and an independent validation against public threat-intelligence sources finds no external support for any checkable misattribution claim in this dataset, including two cases in which the named family does not appear to exist at all.
